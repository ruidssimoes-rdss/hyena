...
By default, Hyena respects the user's system preference:
...
<InfoCard title="System Detection">
  - **iOS**: Follows Settings → Display & Brightness
  - **Android**: Follows Settings → Display → Dark theme
...
```tsx
import { useColorScheme } from '@hyena-studio/react-native'
...
By default, Hyena persists the user's theme preference:

```tsx
<ThemeProvider
  defaultTheme="system"
  storageKey="my-app-theme"  // AsyncStorage key (default: 'hyena-theme')
>
  {children}
</ThemeProvider>
```
...
All Hyena components automatically respect the current theme:
...
import { useColorScheme, Text } from '@hyena-studio/react-native'
...
A more polished toggle using Hyena components:

```tsx
import { Button, useColorScheme } from '@hyena-studio/react-native'
...
```tsx
import { Select, useColorScheme, View, Text } from '@hyena-studio/react-native'
...
Hyena includes a script to prevent theme flash on page load (web only):

```html
<!-- Add to <head> before other scripts -->
<script>
  (function() {
    const stored = localStorage.getItem('hyena-theme')
