...
Hyena components are designed to work across iOS, Android, and Web via React Native and React Native Web. This guide covers platform-specific behaviors and setup requirements.
...
Hyena uses platform-appropriate shadow implementations:
...
```js
// next.config.js
const withTM = require('next-transpile-modules')(['@hyena-studio/react-native']);
...
Hyena works with Expo without additional configuration:

```bash
npx expo install @hyena-studio/react-native react-native-safe-area-context
```
