...
Wrap Hyena components to create your own variants:
...
Build higher-level components from Hyena primitives:
...
```tsx
import { Button as BaseButton, ButtonProps as BaseButtonProps } from '@hyena-studio/react-native'
...
Override Hyena tokens via `tailwind.config.js`:
...
1. Copy the component source from `node_modules/@hyena-studio/react-native/src/components/`
2. Place in your project's `components/ui/` directory
3. Modify as needed
4. Import from your local version instead of `@hyena-studio/react-native`
...
<InfoCard title="Fork Considerations" variant="warning">
  ...
  - Updates to Hyena won't automatically apply to your fork
...
export * from '@hyena-studio/react-native'
```
