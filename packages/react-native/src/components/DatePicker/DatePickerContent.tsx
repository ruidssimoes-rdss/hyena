...
export interface DatePickerContentProps {
  /** Content (DatePickerHeader, DatePickerCalendar, etc.) */
...
export function DatePickerContent({ children, style }: DatePickerContentProps) {
  const { open, setOpen, triggerLayout } = useDatePicker();
  ...
  // Position dropdown below trigger, matching trigger width (or use calendar's natural width)
  const calendarWidth = Math.max(triggerLayout.width, 320);
